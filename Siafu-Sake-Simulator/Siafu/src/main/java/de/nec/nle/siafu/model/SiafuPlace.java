 ...
 * 
 */
public class SiafuPlace extends Place implements Trackable, Publishable, Overlayable {
	 ...
	 */
	public static void initialize(final SiafuWorld newWorld) {
		SiafuPlace.world = newWorld;
		...
		gradients =	new SiafuGradientCache(Controller.DEFAULT_GRADIENT_PATH,
						world.getWorldName(), SiafuWorld.getCacheSize(),
						SiafuWorld.shouldPrefillCache());
	}
	 ...
	 */
	public SiafuPlace(final String type, final SiafuPosition pos, final SiafuWorld world,
			final SiafuPosition relevantPosition) {
	...
	public SiafuPlace(final String type, final SiafuPosition pos, final SiafuWorld world,
			final String name, final SiafuPosition relevantPosition) {
		basicChecks(world);
	 ...
	 */
	private void basicChecks(final SiafuWorld thisPlacesWorld) {
		if (gradients == null) {
			throw new InitializationRequiredException(
					"You need to initialize the Place class");
		}
		if (SiafuPlace.world != thisPlacesWorld) {
	 ...
	 */
	public SiafuPlace(final FlatData flatData) {
		...
		this.pos = new SiafuPosition(lat, lon);
		
		this.info = new TreeMap<String, Publishable>(); 
		world.addPlaceType(type); 
		if (!gradients.containsKey(pos.toString())) { 
		gradients.put(pos, new Gradient((SiafuPosition)pos, world));
		}
	}
	 ...
	 */
	public int pointFrom(final SiafuPosition targetPos) {
	 ...
	 *         0 is North, and 7 is North-West, or -1 if we are there
	 */
	public int pointFrom(final SiafuPosition targetPos, final int preferedDir) {
	 ...
	 *         diagonal step is 14.
	 */
	public int distanceFrom(final SiafuPosition targetPos) {
	...
	public FlatData flatten() {
		String data;
		double[] coords = ((SiafuPosition)pos).getCoordinates();
	...
	public FlatData getContext(final String ctxName)
		...
		if (info.containsKey(ctxName)) {
			...
			return world.getOverlays().get(ctxName).getValue((SiafuPosition)pos)
					.flatten();
