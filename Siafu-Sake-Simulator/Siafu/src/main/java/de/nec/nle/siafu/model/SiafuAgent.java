 ...
 * 
 */
public class SiafuAgent extends Agent implements Trackable, Comparable<SiafuAgent> {
	/** A destination, just to make sure it's never null.*/
	private static SiafuPlace DEFAULT_DESTINATION = null;
	
	private static SiafuPlace getDefaultPlace(SiafuPosition pos, SiafuWorld world){
		if (DEFAULT_DESTINATION == null){
			DEFAULT_DESTINATION = new SiafuPlace("StartingPosition", pos, world);
		}
	...
	private static SiafuWorld world;
	 ...
	 */
	private SiafuPlace destination;
	 ...
	 */
	private void basicChecks(final SiafuWorld thisAgentsWorld) {
		if (SiafuAgent.world == null) {
			throw new InitializationRequiredException(
					"You need to initialize the agents.");
		}
		if (SiafuAgent.world != thisAgentsWorld) {
	 ...
	 */
	public SiafuPlace getDestination() {
		return destination;
	}
	 ...
	 */
	public void setDestination(final SiafuPlace destination) {
	 ...
	 */
	public SiafuPosition getPos() {
		return (SiafuPosition)pos;
	}
	 ...
	 */
	public void setPos(final SiafuPosition pos) {
		this.pos = pos;
	}
	...
	public void moveInDirection(final int moveDir)
		...
		pos = ((SiafuPosition)pos).calculateMove(moveDir);
		this.dir = moveDir;
	}
	 ...
	 */
	public void moveTowardsPlace(final SiafuPlace place) {
		try {
			moveInDirection(place.pointFrom((SiafuPosition)pos, dir));
	 ...
	 * @see #wander()
	 */
	public void wanderAround(final SiafuPlace place, final int radius,
			final int soberness) {
		atDestination = true; // Keep the developer from forgetting this (comm
		// by Kostas)
		if (place.distanceFrom((SiafuPosition)pos) > radius) {
	 ...
	 * @see #wanderAround(SiafuPlace, int, int)
	 */
	public void wanderAround(final SiafuPlace place, final int radius) {
	...
	public void wander(final int soberness) {
		...
		while (stuck && (tries < POSSIBLE_DIRECTIONS)) {
			SiafuPosition target = null;

			try {
				target = ((SiafuPosition)pos).calculateMove(dir);
				pos = target;
				stuck = false;
			} catch (PositionUnreachableException e) {
				turn(searchDir);
	...
	public FlatData getContext(final String ctxName)
		...
		if (info.containsKey(ctxName)) {
			...
			return world.getOverlays().get(ctxName).getValue((SiafuPosition)pos).flatten();
			...
			return ((SiafuPosition)pos).flatten();
	 ...
	 */
	public static void initialize(final SiafuWorld agentsWorld) {
		world = agentsWorld;
		infoFieldsLocked = false;
	}

	public int compareTo(SiafuAgent a) {
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SiafuAgent other = (SiafuAgent) obj;
	...
	public void setPosition(int col, int row) {
		this.pos = new SiafuPosition(col, row);
	}
}
