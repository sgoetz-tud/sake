 ...
 *
 */
public class ExploreMergeMasterCalculateRelativeFactory extends ISimulationFactory {
	...
	public ArrayList<Agent> createRobots(AgentFactory factory) {
		...
		if (configuration.getWc().number_explore_agents > 0) {
			...
			for (int i = 0; i < configuration.getWc().number_explore_agents; i++) {
				...
				FollowerRole fre = new ExplorerRelativeFollowerRole(((ICoreAdapter)era).getAgentCore(), mre);
				fre.addRole(fre);
			...
			if (configuration.getWc().number_hoove_agents > 0) {
				...
				for (int i = 0; i < configuration.getWc().number_hoove_agents; i++) {
					...
					FollowerRole frh = new HooverRelativeFollowerRole(((ICoreAdapter)hra).getAgentCore(), mrh);
					frh.addRole(frh);
				...
				if (configuration.getWc().number_wipe_agents > 0) {
					...
					for (int i = 0; i < configuration.getWc().number_wipe_agents; i++) {
						...
						FollowerRole frw = new WiperRelativeFollowerRole(((ICoreAdapter)wra).getAgentCore(), mrw);
						frw.addRole(frw);
