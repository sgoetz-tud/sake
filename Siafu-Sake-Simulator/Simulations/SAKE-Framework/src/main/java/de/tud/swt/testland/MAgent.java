 ...
 *
 */
public class MAgent extends Agent implements ISimulatorAdapter, ICoreAdapter {

	private boolean finish;	
	private AgentCore cleaningRobot;
	private World siafuWorld;
	
	public MAgent(String name, String image, World world, Configuration configuration) {
		super(name, world.getColFromStart(), world.getRowFromStart());
	...
	public List<AgentCore> getNearRobots(int visionRadius) {
		...
		for (Agent nearAgent : this.siafuWorld.getAgents())
		{
			if (Math.abs(this.getCol() - nearAgent.getCol()) <= visionRadius 
				...
				result.add(((MAgent)nearAgent).getAgentCore());
			}
		}
		return result;
	}
