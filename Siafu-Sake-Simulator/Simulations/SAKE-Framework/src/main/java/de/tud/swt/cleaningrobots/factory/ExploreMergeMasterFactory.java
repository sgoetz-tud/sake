 ...
 *
 */
public class ExploreMergeMasterFactory extends ISimulationFactory {
	...
	public ArrayList<Agent> createRobots(AgentFactory factory) {
		...
		if (configuration.getWc().number_explore_agents > 0) {
			MasterRole mre = new MergeMasterRole(((ICoreAdapter)lsa).getAgentCore());
			mre.addRole(mre);		
			...
			if (configuration.getWc().number_hoove_agents > 0) {
				MasterRole mrh = new MergeMasterRole(((ICoreAdapter)lsa).getAgentCore());
				mrh.addRole(mrh);
				...
				if (configuration.getWc().number_wipe_agents > 0) {
					MasterRole mrw = new MergeMasterRole(((ICoreAdapter)lsa).getAgentCore());
					mrw.addRole(mrw);
